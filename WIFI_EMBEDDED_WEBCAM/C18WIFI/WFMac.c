...
WORD  header;
 ...
 header = MACGet(); header <<= 8; header |=MACGet();
return  header;
...
WORD ToInt16 (BYTE valueH,BYTE valueL)
{
   WORD result;
...
void ne_send(WORD len, BYTE packet) {
  BYTE  i;	
  if(packet==1)
    i=1;

  g_txPacketLength=len + WF_TX_PREAMBLE_SIZE;
